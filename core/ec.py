...
# --- THE CURVE RULES (secp256k1) ---
# P is a massive prime number that defines the field 'boundaries'.
...
G = (x, y)
# n is the total number of points available on the curve (the order).
n = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141

def pointAdd(p: tuple, q: tuple):
    """     Geometric Addition:   In ECC, 'adding' two points isn't simple math. You draw a line through 
    ...
    if p[0] == q[0]:
        if p[1] != q[1]: # Points are vertical mirror images; they 'cancel out'
            return None
        # CASE: Point Doubling (P == Q). We find the tangent line at that point.
        lam = f_mod((3 * mul(p[0], p[0])) * modinv(2 * p[1], P), P)
    else:
        # CASE: Standard Addition. Find the slope between two different points.
...
def scalMul(k: int, p: tuple):
    """  The 'Double and Add' Algorithm:
         Multiplies a point P by a number k. This is the 'One-Way' function 
         that makes ECC secure. It's easy to multiply, but impossible to undo.  """
    result = None
    binSum = p
    while(k > 0):
...
def onCurv(p: tuple):
    """The Security Guard: Checks if a point actually sits on y^2 = x^3 + 7."""
    left = mul(p[1], p[1])
    right = mul(mul(p[0], p[0]), p[0]) + b
    return f_mod(left - right, P) == 0

def xyTob64(p: tuple):
    """Converts a (x, y) math coordinate into a shareable Base64 string."""
