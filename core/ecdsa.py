...
def genKeys():
    ...
    p = secrets.randbelow(n-1) + 1  # secrets.randbelow is used for 'true' randomness (cryptographically secure)
    return (p, scalMul(p, G))

def sign(m: bytes, d: int):
    """  The Digital Seal:   Signs a message 'm' (usually the Merkle Root) using a Private Key 'd'.
...
# 2. Turn the message hash into a big integer (z)
    z = f_mod(int.from_bytes(sha256(m).digest(), 'big'), n)

    r = scalMul(k, G)[0]  # 3. r is the x-coordinate of the point k*G
# 4. s = k⁻¹ * (z + r*d) mod n
# This is the 'magic' formula that binds the message, the identity, and the nonce.
    s = f_mod(mul(modinv(k, n), z + mul(r, d)), n)
    
    return (r, s)

def verify(m: bytes, sgn: tuple, Q: tuple):
    """  The Auditor:    Checks if a signature (r, s) is valid for message 'm' using Public Key 'Q'.    """
